...
void TopUI::OrbDrawAndLogic(Health counter, int factor, int fullIndex, int halfIndex, int emptyIndex, Vector2D offset)
{
	...
	while (counter.GetBaseHealth() > 0)
	{
		...
		Engine::GetInstance().render->SelectLayer(5);
...
void TopUI::Render()
{
	...
	Engine::GetInstance().text->Write("-gold-    -item-    -life-                  -boss-", -Engine::GetInstance().render->camera.x, -Engine::GetInstance().render->camera.y, 5);
	Engine::GetInstance().text->Write(std::to_string(Engine::GetInstance().scene->player->coins.GetAmount()).c_str(), -Engine::GetInstance().render->camera.x + 16, -Engine::GetInstance().render->camera.y  + 8,5);
	Engine::GetInstance().text->Write(std::to_string(Engine::GetInstance().scene->player->magic.GetAmount()).c_str(), -Engine::GetInstance().render->camera.x + 96, -Engine::GetInstance().render->camera.y  + 8, 5);
}
